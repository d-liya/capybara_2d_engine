...
- **Art Style Consistency**: Use the batch-level `artStyle` field to specify global aesthetic rules (e.g., _"modern pixel art, vibrant color palette"_). Do not repeat these style phrases inside individual asset prompts. **Default**: if the user's prompt does not name an art style, set `artStyle` to modern pixel art.
